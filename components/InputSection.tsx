...
const PERSONAS: { id: HiringPersona; label: string; icon: string; desc: string }[] = [
...
const InputSection: React.FC<InputSectionProps> = ({ onAnalyze, isAnalyzing, darkMode, addToast }) => {
  ...
  const [persona, setPersona] = useState<HiringPersona>('Standard');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if ((resumeText.trim() || resumeFile) && jobDescription.trim()) {
      onAnalyze({ resumeText, resumeFile, jobDescription, persona });
    } else {
      setShake(true);
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 py-8">
      <div className="text-center mb-10 animate-fade-in">
        <h1 className={`text-6xl font-extrabold tracking-tight mb-4 ${darkMode ? 'text-white' : 'text-slate-900'}`}>
        ...
        <p className={`text-lg max-w-2xl mx-auto transition-colors duration-300 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
        ...
        <div className="mt-4 flex justify-center gap-4 opacity-0 animate-slide-up" style={{ animationDelay: '0.2s' }}>
           <button onClick={loadDemoData} className="text-xs px-3 py-1 rounded border border-indigo-300 text-indigo-500 hover:bg-indigo-50 transition-colors">
             Use Demo Data
           </button>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className={`grid grid-cols-1 md:grid-cols-2 gap-8 ${shake ? 'animate-shake' : ''}`}>
        ...
        <div className="space-y-4 opacity-0 animate-slide-up" style={{ animationDelay: '0.3s' }}>
          ...
          <div 
            ...
            onDrop={handleDrop}
            className={`
            relative group w-full h-80 rounded-2xl border border-dashed transition-all duration-300 overflow-hidden flex flex-col hover-card
          ...
          `}>
              ...
              {resumeFile || extractionPreview || resumeText ? (
                <div className="flex items-center w-full animate-pop">
                  ...
                  <button 
                    type="button"
                    onClick={clearFile}
                    className="text-slate-400 hover:text-red-500 p-1 transition-colors hover:scale-110"
                  >
            ...
            {resumeFile ? (
              ...
              <textarea
                ...
                className={`flex-1 w-full p-4 resize-none font-mono text-sm leading-relaxed focus:outline-none transition-colors ${darkMode ? 'bg-slate-800 text-slate-300' : 'bg-white text-slate-700'}`}
        ...
        <div className="space-y-4 opacity-0 animate-slide-up" style={{ animationDelay: '0.4s' }}>
          ...
          <div className="relative group">
            <textarea
              ...
              className={`w-full h-80 p-4 rounded-2xl border transition-all duration-300 resize-none font-mono text-sm leading-relaxed hover-card
        ...
        <div className="md:col-span-2 space-y-6 opacity-0 animate-slide-up" style={{ animationDelay: '0.5s' }}>
           ...
           <div className="max-w-4xl mx-auto">
              ...
              <p className={`text-center text-xs mt-3 italic h-4 transition-all duration-300 ${darkMode ? 'text-slate-500' : 'text-slate-400'}`}>
           ...
           <div className="flex flex-col items-center justify-center pt-8 pb-10">
            <button
              ...
              className={`
                relative overflow-hidden group px-16 py-6 rounded-full text-xl font-bold text-white shadow-2xl transition-all transform duration-300
            ...
            {isAnalyzing && (
              <p className="mt-6 text-sm text-indigo-500 animate-pulse font-medium bg-indigo-50 px-4 py-2 rounded-full shadow-sm">
